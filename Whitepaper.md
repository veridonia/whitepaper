...
**How many are asked, and how many must answer.** Both are read from the community rather than set for it, and both are read against a clock. The bar is what that community can be relied on to produce within the time a decision is meant to take — not what it manages on an average day, since an average is cleared barely half the time, and not how often its members answer eventually, since almost everyone answers eventually and eventually is not a decision. The panel is then the fewest people who clear that bar as reliably; where a community is too small for that, everyone is asked and the decision falls back on its deadline. A bar above what a community can supply does not raise its standards; it only means every decision runs to its deadline. Panel size and quorum saturate rather than growing with the community: past a handful of votes a panel learns very little more about what a population thinks.
